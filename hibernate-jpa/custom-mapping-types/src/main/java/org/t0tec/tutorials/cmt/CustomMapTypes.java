...
public class CustomMapTypes {
  ...
  public void doFirstUnit() {
    ...
    Item sameItem =
        new Item("Blue&White Dell Laptop", "15\" laptop with Intel Core i5",
            MonetaryAmount.convert(maEUR, Currency.getInstance("USD")));
  ...
  public void doSecondUnit() {
    ...
    session.save(new Item("Logitec USB Wireless Mouse",
        "Wireless USB mouse with Logitech Unifying software", new MonetaryAmount(
            new BigDecimal(50), Currency.getInstance("EUR"))));
    ...
    List<Item> euroItems =
        listAndCast(newSession
            .createQuery("from Item i where i.initialPrice.value > 40 and i.initialPrice.currency = 'EUR' order by i.initialPrice.value asc"));
  ...
  private void getAllItems() throws HibernateException {
    ...
    List<Item> items =
        listAndCast(newSession.createQuery("from Item i order by i.initialPrice.value asc"));
